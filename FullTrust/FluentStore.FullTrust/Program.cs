using FluentStore.FullTrust.MessageHandlers;
...
namespace FluentStore.FullTrust
{
    ...
    {
        public const string AppServicePipeName = "FluentStoreFullTrust_ServerPipe";
        ...
        private static void UnhandledExceptionTrapper(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            Logger.UnhandledException(exception, exception.Message);
        }
        ...
        {
            connection = new NamedPipeServerStream(AppServicePipeName, PipeDirection.InOut, NamedPipeServerStream.MaxAllowedServerInstances, PipeTransmissionMode.Message, PipeOptions.Asynchronous, 2048, 2048, null, HandleInheritability.None, PipeAccessRights.ChangePermissions);
        ...
        private static async Task ParseArgumentsAsync(Dictionary<string, object> message, string arguments)
        {
            switch (arguments)
            {
                ...
                case "Elevate":
                    ...
                    if (message.TryGetValue("RequestID", out var requestIdObj))
                    ...
                    if (!IsAdministrator())
                    {
                        try
                        {
                            ...
                            await Win32API.SendMessageAsync(connection, new ValueSet() { { "Success", 0 } }, requestId);
                            appServiceExit.Set();
                        }
                        catch (Win32Exception)
                        {
                            // If user cancels UAC
                            await Win32API.SendMessageAsync(connection, new ValueSet() { { "Success", 1 } }, requestId);
                        }
                    }
                    else
                    {
