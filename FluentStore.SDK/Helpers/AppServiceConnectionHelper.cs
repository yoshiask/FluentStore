...
namespace FluentStore.SDK.Helpers
{
    ...
    {
        private static LoggerService Logger = Ioc.Default.GetRequiredService<LoggerService>();
        ...
        public static async Task<bool> Elevate(this NamedPipeAsAppServiceConnection connection)
        {
            ...
            {
                var res = response.GetOrDefault("Success", 1L);
                switch (res)
                {
        ...
        private static async Task<NamedPipeAsAppServiceConnection> BuildConnection(bool launchFullTrust)
        {
            try
            {
                ...
                if (await connection.Connect(@$"\\.\pipe\{AppServicePipeName}", TimeSpan.FromSeconds(15)))
                {
                    return connection;
                }
                connection.Dispose();
            ...
            {
                Logger.Warn(ex, "Could not initialize FTP connection!");
            }
            return null;
        }
    }
    ...
    {
        private LoggerService Logger = Ioc.Default.GetRequiredService<LoggerService>();
        ...
        private void EndReadCallBack(IAsyncResult result)
        {
            ...
            if (readBytes > 0)
            {
                ...
                else // Message is completed
                {
                    ...
                    if (msg.GetOrDefault("RequestID", (string)null) == null)
                    {
        ...
        public async Task<(AppServiceResponseStatus Status, Dictionary<string, object> Data)> SendMessageForResponseAsync(ValueSet valueSet)
        {
            ...
            {
                Logger.Warn(ex, "Error sending request on pipe.");
            }
